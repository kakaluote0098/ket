...
export default function Listening() {
  ...
  const handleAnswer = (correct: boolean) => {
    if (correct) {
      ...
      progress.updateScore('listeningScore', 8);
    }
  ...
  return (
    <Layout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-space-900">听力训练</h1>
          <p className="text-sm text-space-900/60">
            {finished ? '训练完成' : `第 ${index + 1} / ${questions.length} 题`}
          </p>
        </div>
      </div>
      ...
      {!finished ? (
        <>
          ...
          <ListeningCard key={current.id} question={current} onAnswer={handleAnswer} />
        </>
      ) : (
        <div className="mx-auto max-w-md text-center">
          <div className="card flex flex-col items-center gap-4">
            ...
            <h2 className="text-2xl font-bold text-space-900">听力训练完成！</h2>
